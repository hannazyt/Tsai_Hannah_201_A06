...
[https://github.com/hannazyt/Tsai_Hannah_201_A06](https://github.com/hannazyt/Tsai_Hannah_201_A06)
...
for(x in names(player_info)){
  player<-player_info[[x]]
  calculate_points<-player$points/player$games_played
  points_per_game[[x]]<-calculate_points
}
...
```{r}
round_points<-sapply(points_per_game, function(x){round(x, 2)})
round_points
```
...
```{r}
rounding<-function(x){
  digits<-nchar(as.character(abs(x)))
  if(digits<=2){
    round(x,-1)
  }else if(digits<=3){
    round(x,-2)
...
round_num<-apply(mat_rand, MARGIN = 1:2, rounding)
round_num
```
...
```{r}
average_temp<-apply(max_temps[, 3:14], 2, mean)
average_temp
```
...
```{r}
annual_median_temp<-apply(colorado_temp[, 3:14], 1, median)

max_temps<-which.max(annual_median_temp)
min_temps<-which.min(annual_median_temp)


max_med_year<-colorado_temp$year[max_temps]
min_med_year<-colorado_temp$year[min_temps]

print(paste0("The maximum median annual temperature of ",  annual_median_temp[max_temps], " occurred in ", max_med_year, " and the minimum median annual temperature of ", annual_median_temp[min_temps], " occurred in ", min_med_year, "."))
...
digit_sum<-function(num){
  if (num==0){
    ...
    return(last_digit+digit_sum(remaining_number))
  }
}
result<-digit_sum(935068)
